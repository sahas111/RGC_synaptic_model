...
import scipy.io
exc= scipy.io.loadmat('rd-ON-exc-output/rd_ON_exc_output.mat')
inh = scipy.io.loadmat('rd-ON-inh-output/rd_ON_inh_output.mat')
# print sys.path
import pylab
import numpy as np
...
tsp = h.Vector()
vCopy = h.Vector()
ghdbar_hd_syn_rd_ON =h.Vector()
gbar_lva_syn_rd_ON = h.Vector()
gbar_nap_syn_rd_ON=h.Vector()
...
soma.gbar_nap=5e-8
for zz in range(0,1):
    soma.gbar_lva=1e-3#2e-4


    for jj in range(0,10):#13 or 20 #17

        soma.ghdbar_hd=1e-3

        for ii in range(0,25): #13 or 20 #15
            ...
            h.dt=0.025
            h.v_init = -60#-53
            h.tstop = 100000#50000
            h.init()
            h.run()
            sizetsp=tsp.size()

            StartTimeBlock1=0+100
            StartTimeBlock2=100000 #50000
            ...
            if (v1NoSpikeMean>=-82 and v1NoSpikeMean<=-47):
                v1NoSpikeMean01=1
            ...
            if (FreqMean1>=2 and FreqMean1<=8):
                FreqMean1_S=1
            ...
            if (FreqMean1_S==1 and v1NoSpikeMean01==1):
                ...
                # f.write( "%g %g %g" %soma.gbar_nap %soma.gbar_lva %soma.ghdbar_hd)
                ghdbar_hd_syn_rd_ON.append(soma.ghdbar_hd)
                gbar_lva_syn_rd_ON.append(soma.gbar_lva)
                gbar_nap_syn_rd_ON.append(soma.gbar_nap)
...
# plt.plot(gbar_lva_syn_rd_ON, ghdbar_hd_syn_rd_ON,linestyle='', marker='^',markersize=10,markerfacecolor='blue')
# plt.show()


gbar_lva_syn_rd_ON1 = np.asarray(gbar_lva_syn_rd_ON,dtype = 'float64')
ghdbar_hd_syn_rd_ON1 = np.asarray(ghdbar_hd_syn_rd_ON,dtype = 'float64')
...
outputfile = 'rd-ON-final'
completeName = os.path.join(save_path, outputfile+".txt")
text=np.savetxt(completeName,zip(gbar_lva_syn_rd_ON1,ghdbar_hd_syn_rd_ON1))


# file= 'simulation_results/rd-ON_2.txt'
# f = open(file, 'r')
# lines=f.readlines()
#
# gbar_lva_syn_rd_ON =[]
# ghdbar_hd_syn_rd_ON = []
# for x in lines:
#     gbar_lva_syn_rd_ON.append(x.split()[0])
#     ghdbar_hd_syn_rd_ON.append(x.split()[1])
#
#     f.close()
#
# gbar_lva_syn_rd_ON1 = np.asarray(gbar_lva_syn_rd_ON,dtype = 'float64')
...
#
#
#     for jj in range(0,28):#13 or 20 #17
#
#         soma.ghdbar_hd=0.000118
#
#         for ii in range(0,35): #13 or 20 #15
#
#             start_time = time.time()
#
#
...
#             soma.ghdbar_hd=soma.ghdbar_hd+.000001
#
#
#         soma.gbar_lva=soma.gbar_lva + .00001
#
#     #soma.gbar_nap=soma.gbar_nap*10
#
#
# gbar_lva_all1 = np.asarray(ghdbar_hd_all,dtype = 'float64')
...
# plt.plot(gbar_lva_syn_rd_ON, ghdbar_hd_syn_rd_ON,linestyle='', marker='^',markersize=7,markerfacecolor='blue')
# plt.show()
#
#
# save_path = 'simulation_results'
# outputfile = 'rd-ON-voltage_trace_2'
# completeName = os.path.join(save_path, outputfile+".txt")
# text=np.savetxt(completeName,zip(t_vec,vCopy))
#
#
