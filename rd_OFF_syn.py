...
for zz in range(0,1):
    ...
    for jj in range(0,20):#20

        soma.ghdbar_hd=0#2e-6

        for ii in range(0,18): #18
            ...
            if (v1NoSpikeMean>=-56 and v1NoSpikeMean<=-50):
                v1NoSpikeMean01=1
            ...
            if (FreqMean1>=8 and FreqMean1<=16):
                FreqMean1_S=1
...
gbar_lva_syn_rd_OFF1 = np.asarray(gbar_lva_syn_rd,dtype = 'float64')
ghdbar_hd_syn_rd_OFF1 = np.asarray(ghdbar_hd_syn_rd,dtype = 'float64')
...
outputfile = 'rd-OFF-final_with_different_seed-point'
completeName = os.path.join(save_path, outputfile+".txt")
text=np.savetxt(completeName,zip(gbar_lva_syn_rd_OFF1,ghdbar_hd_syn_rd_OFF1))


import matplotlib.pyplot as plt

file= 'simulation_results/rd-OFF-final.txt'
f = open(file, 'r')
lines=f.readlines()
